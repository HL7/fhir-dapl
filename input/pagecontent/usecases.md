...
The section identifies the business needs and specific user stories for DAPL FHIR IG. 
The same set of business needs are applicable to the DARTS IG.
...
### Explanation of Psuedonymized, Deidentified and Anonymized data

This section repeats some of the definitions from the DARTS IG.
...
The above definitions are used to create the necessary profiles to represent the data and the conventions are defined below.
...
#### Profiles common to Deidentification and Anonymization 

Many of the profiles that are defined in the IG are common to both deidentification and anonymization. For e.g, profiles related to Diagnosis, Procedures etc. 

These profiles will be named as dapl-[Resource/Profile Name]" where the Resource Name would be "Procedure or MedicationRequest etc".

Sometimes instead of Resource name the IG uses a profile name which is unique. For e.g, lab-observation, income-observation where the Observation resource is profiled multiple times for different purposes.  

These profiles which dont have the words "deidentified" or "anonymized" can be used for representing both Deidentified data and Anonymized data.

#### Profiles used only to represent Deidentified data

Some profiles are to be used only to represent deidentified data which carry certain information about age, sex, race, ethnicities, locations with a low risk of reidentification.  

These profiles will be named as dapl-deidentified-[Resource/Profile Name]" where the Resource Name would be "Patient" or "Encounter".

#### Profiles used only to represent Anonymized data

Some profiles are to be used only to represent anonymized data which eliminates/generalizes age, sex, race, ethnicities, locations further (beyond deidentified profiles) to eliminate reidentification risk  

These profiles will be named as dapl-anonymized-[Resource/Profile Name]" where the Resource Name would be "Patient" or "Encounter".
